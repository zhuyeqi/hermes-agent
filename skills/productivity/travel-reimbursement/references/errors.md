...
| `session_conflict` | Gate.A / login gate2：cookie probe 未认证（见 stderr probe JSON） | **exit 1**；对照 `references/login.md` gate2 段；可能他处登录 — 重跑 login，勿改 ITEMS_JSON |
...
| `items_enum_invalid` | vehicle / invoice_form / city_type 等不在 `erm_travel_enums.py` | **停机**；改 JSON 为字典精确 key（见 stderr `suggestions` / `allowed_*`） |
| `items_json_invalid` | JSON 格式/缺字段 | 修复后重跑 `preflight_check.sh` |
| `gate_failed` | Gate.B–E | 读 `RUN_DIR` 产物；勿猜测 dispatch 字段 |

## 防发散（硬性）

1. **预检失败** → 只修预检指出的项，不跑 login/pipeline。
2. **exit 3** → 不尝试 savebill、不改 ITEMS_JSON 枚举、不换账号 profile。
3. **wrong_password** → 不向用户索要密码以外的 workaround。
4. **items_enum_invalid** → 不擅自用最相近枚举提交；须用户确认显示名与 `scripts/lib/erm_travel_enums.py` 的 key 完全一致。
...
```
preflight_check.sh → lib/{init,erm_workspace,resolve_python_env}.sh, check_browser_health.sh, validate_travel_items_enums.py
...
- `scripts/validate_travel_items_enums.py` — 差旅枚举与别名提示
