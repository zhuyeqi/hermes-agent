...
#
# Optional env:
#   ERM_USERID    - if unset, prompts interactively
#   ERM_PASSWORD  - if unset, prompts interactively (hidden)
#
# Exit codes:
...
[[ -n "$ERM_BASE_URL" ]] || die "failed to resolve ERM_BASE_URL"
...
# Collect credentials if not provided via env
if [[ -z "${ERM_USERID:-}" || -z "${ERM_PASSWORD:-}" ]]; then
  die "请提供账号(ERM_USERID)和密码(ERM_PASSWORD)，通过环境变量设置后重新运行。"
fi
...
# Parse snapshot: use "密码" label as anchor.
#   - textbox before "密码" = userid
#   - textbox after "密码"  = password
#   - generic "登录"        = submit button
REFS="$(echo "$SNAPSHOT" | python3 -c '
import re, sys, json

snapshot = sys.stdin.read()

# Collect all elements with refs in order of appearance
elements = []
...
# password = first textbox after "密码"
password_ref = None
...
# Find submit button: generic clickable with "登录" text
submit_ref = None
for el in elements:
...
if [[ $? -ne 0 ]]; then
  die "failed to parse snapshot. Raw output:\n$SNAPSHOT"
fi

USERID_REF="$(echo "$REFS" | python3 -c "import json,sys;print(json.load(sys.stdin)['userid_ref'])")"
PASSWORD_REF="$(echo "$REFS" | python3 -c "import json,sys;print(json.load(sys.stdin)['password_ref'])")"
SUBMIT_REF="$(echo "$REFS" | python3 -c "import json,sys;print(json.load(sys.stdin)['submit_ref'])")"
...
# Gate 1: URL must not contain login.jsp
